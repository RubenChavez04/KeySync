...
def run_flask():
    ...
    try:
        flask_app.run(port=2266, debug=False, use_reloader=False)
...
async def run_everything():
    #start the websocket server task in the asyncio loop without blocking main program
...
def main():
    ...
    window.show()
    window.init()
    #run server tasks within the asyncio loop
